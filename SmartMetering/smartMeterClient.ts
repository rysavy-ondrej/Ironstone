...
class SmartMeterClient {
    ...
    private loadSourceFile = (path:string, columns: {from:number, to:number}) => {
        ...
        fast_csv.fromStream(stream, {headers : true, objectMode : true, delimiter : ';'})
            .on("data", function(data : any){
                    ...
                    let array :string [] =  Object.keys(data).map(function (key) { return data[key]; });
     ...
     */
    private initialize = () =>  {
        client.init({client: this.config}, function () {
        ...
        // serial number
        client.registry.setResource('/3/0', '2', this.config.serialNumber.toString(), function (error, value) { if (error) handleError(error); } );
